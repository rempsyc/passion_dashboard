...
partial_vlookup <- function(pattern, lookup_vector) {
  out <- map_chr(pattern, \(x) {
    out <- grep(x, lookup_vector, ignore.case = TRUE, value = TRUE)[1]
    if (is.na(out)) {
      out <- rgrep(lookup_vector, x, ignore.case = TRUE, value = TRUE)[1]
    }
    out
    })
  out
...
list.countries <- c(unique(countryname_dict$country.name.en), "USA", "Korea", "UK")
get_country <- function(x, list.countries) {
  rgrep(list.countries, x, ignore.case = TRUE, value = TRUE)[1]
}
